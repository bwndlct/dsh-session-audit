...
**dsh-session-audit performs local analysis only.** 只读取本机 DSH home
的 session 日志和内存中的 live session 注册表。不发起任何网络请求、
不调 LLM API、不发遥测、不收集任何用户数据。报告只作为工具结果存在
于你自己的会话里。
...
重复行为、验证情况。截至 v0.1.0，`dsh-plugin` 生态中没有其他覆盖
单 session 执行审计的插件。
