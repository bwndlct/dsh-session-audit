 ...
 */
const PATTERNS: readonly VerificationPattern[] = [
...
export function matchVerificationCommand(command: string): { type: VerificationType; command: string } | undefined {
	for (const pattern of PATTERNS) {
		const match = pattern.re.exec(command)
		if (match !== null) {
			const matched = match[0].trim()
			if (matched !== '') return { type: pattern.type, command: matched }
		}
	}
	return undefined
}
...
export function shellCommandOf(call: ToolCallRecord, t: AuditThresholds): string | undefined {
	if (!t.shellTools.includes(call.name)) return undefined
...
export function exitCodeMarkerOf(text: string): number | undefined {
 ...
 */
export function shellCallSucceeded(call: ToolCallRecord): boolean {
	const result = call.result
	if (result === undefined) return false // no durable result: outcome unknown → not a success
...
export function detectVerification(facts: AuditFacts, t: AuditThresholds): VerificationResult[] {
	const byCommand = new Map<string, VerificationResult>()
	for (const call of facts.toolSequence) {
		if (call.result === undefined) continue // unresolved call: outcome unknown, not an attempt
		...
		if (entry === undefined) {
			...
			entry.attempts += 1
			if (ok) entry.okAttempts += 1
			else entry.failedAttempts += 1
			entry.lastOk = ok
		}
	}
...
export function evaluateVerification(results: VerificationResult[], t: AuditThresholds): AuditSignal[] {
