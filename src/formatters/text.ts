...
export function formatTextReport(report: SessionAuditReport): string {
	const lines: string[] = []
	...
	if (report.session.model !== undefined) push(label('Model'), report.session.model)
	if (report.session.provider !== undefined) push(label('Provider'), report.session.provider)
	if (report.session.cwd !== undefined) push(label('Cwd'), report.session.cwd)
	push(label('Duration'), `${formatDuration(report.session.durationMs)}${report.execution.openTurn ? ' (turn still open — live session)' : ''}`)
	if (report.session.startedAt !== undefined) push(label('Started'), report.session.startedAt)
	push()
	...
	if (report.tools.unresolvedCalls > 0) push(label('Unresolved'), String(report.tools.unresolvedCalls))
	...
	if (report.tokens === undefined) {
		...
		if (report.tokens.cacheReadTokens > 0) push(label('Cache read'), formatNumber(report.tokens.cacheReadTokens))
		if (report.tokens.cacheWriteTokens > 0) push(label('Cache write'), formatNumber(report.tokens.cacheWriteTokens))
		if (report.tokens.reasoningTokens > 0) push(label('Reasoning'), formatNumber(report.tokens.reasoningTokens))
	...
	if (report.signals.length === 0) {
		...
		for (const signal of report.signals) {
			...
			if (signal.detail !== undefined) push(`     ${signal.detail}`)
		}
	}
	push()
