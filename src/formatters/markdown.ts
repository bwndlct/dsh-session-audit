 ...
 *
 * @module dsh-session-audit/formatters/markdown
 */
import type { SessionAuditReport } from '../audit/types.ts'
import { formatDuration, formatNumber, formatPercent } from '../utils/duration.ts'

export function formatMarkdownReport(report: SessionAuditReport): string {
	const lines: string[] = []
	lines.push('# DSH Session Audit', '')
	lines.push('## Session', '')
	lines.push(`- **ID**: \`${report.session.id}\``)
	if (report.session.model !== undefined) lines.push(`- **Model**: ${report.session.model}`)
	if (report.session.provider !== undefined) lines.push(`- **Provider**: ${report.session.provider}`)
	if (report.session.cwd !== undefined) lines.push(`- **Cwd**: \`${report.session.cwd}\``)
	lines.push(
		`- **Duration**: ${formatDuration(report.session.durationMs)}${
			report.execution.openTurn ? ' *(turn still open — live session)*' : ''
		}`,
	)
	if (report.session.startedAt !== undefined) lines.push(`- **Started**: ${report.session.startedAt}`)
	lines.push('')
	...
	if (report.tools.unresolvedCalls > 0) lines.push(`- **Unresolved**: ${report.tools.unresolvedCalls}`)
	...
	if (report.tokens === undefined) {
		...
		lines.push('')
		lines.push(
			'dsh records token usage only when the provider reports it; the provider used by this session did not. No values are estimated.',
		)
	} else {
		...
		if (report.tokens.cacheReadTokens > 0) lines.push(`| Cache read | ${formatNumber(report.tokens.cacheReadTokens)} |`)
		if (report.tokens.cacheWriteTokens > 0) lines.push(`| Cache write | ${formatNumber(report.tokens.cacheWriteTokens)} |`)
		if (report.tokens.reasoningTokens > 0) lines.push(`| Reasoning | ${formatNumber(report.tokens.reasoningTokens)} |`)
	...
	if (report.verification.length === 0) {
		lines.push(
			'No test/build/lint verification command was detected in the observed session. This does not mean no verification happened — it may have run through a mechanism this analysis cannot observe.',
		)
	} else {
	...
	if (quality.skippedEvents > 0 || quality.truncatedFrames > 0 || quality.unknownEventTypes.length > 0) {
		...
		if (quality.truncatedFrames > 0) lines.push(`- ${quality.truncatedFrames} torn log frame(s) skipped (crash tail)`)
		lines.push('')
	}
