    ...
    for (const event of raw.events) {
        ...
        switch (type) {
            case 'turn/start': {
                ...
                if (turn !== undefined)
                    events.push({ kind: 'turn-start', seq, time, turn });
                else
                    skippedEvents += 1;
                break;
            }
            case 'turn/end': {
                ...
                if (turn !== undefined)
                    events.push({ kind: 'turn-end', seq, time, turn, reason });
                else
                    skippedEvents += 1;
                break;
            }
            ...
            default: {
                ...
                if (type === '' || !known) {
                    skippedEvents += 1;
                    if (type !== '' && !unknownEventTypes.includes(type))
...
function firstToolResultBlock(content) {
    if (!Array.isArray(content))
        return undefined;
    for (const entry of content) {
        if (!isRecord(entry) || entry.type !== 'tool-result')
            continue;
        let text;
